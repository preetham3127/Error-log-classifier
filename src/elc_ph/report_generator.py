...
class ReportGenerator:
    ...
    def generate_summary(self, clusters, stats=None):
        ...
        # Prefer exact total_lines from stats (which may include blank lines),
        # otherwise fall back to counting clustered lines.
        total_lines = None
        if stats and isinstance(stats, dict) and 'total_lines' in stats:
            try:
        ...
        report = {
            'summary': {
                ...
                'total_lines': total_lines if total_lines is not None else sum(len(lines) for lines in clusters.values()),
